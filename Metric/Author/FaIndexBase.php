 ...
 */
abstract class FaIndexBase extends AuthorMetric
{
     ...
     * @return mixed
     */
    abstract protected function getPaperAuthorScore($paperId, $authorId, $depth);
            ...
            foreach ($this->authorPapers as $authorId => $papers) {
                ...
                if (!$this->useTopPapers || $numPapers <= $this->topPapers) {
                    foreach ($papers as $paperId) {
                        $scores[$authorId][$depth] += $this->getPaperAuthorScore($paperId, $authorId, $depth);
                    }
                } else {
                    $authorPaperScores = [];
                    ...
                    foreach ($papers as $paperId) {
                        $authorPaperScores[] = $this->getPaperAuthorScore($paperId, $authorId, $depth);
                    }
